...
def calculate_physical_damage(strength, min_weapon_dmg, max_weapon_dmg):
    ...
    return (
        min_weapon_dmg + bonus,
        max_weapon_dmg + bonus
    )
...
def recalculate_character_stats(character):
    ...
    character.max_hp = calculate_max_hp(character.vitality, character.hp_from_levels) + total_hp_bonus
    character.max_mana = calculate_max_mana(character.intelligence, character.mana_from_levels) + total_mana_bonus
    character.max_stamina = calculate_max_stamina(character.dexterity, character.vitality) + total_stamina_bonus
    ...
    character.spell_power = calculate_spell_power(character.intelligence, total_spell_power)
    character.physical_min_damage, character.physical_max_damage = calculate_physical_damage(
        character.strength, weapon_min_dmg, weapon_max_dmg
    )
